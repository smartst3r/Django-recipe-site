	...
	if request.method == 'POST':
		...
		if formset.is_valid():
			...
			for i in formset.cleaned_data:
				...
				templist.append(ing_name)
				templist.append(ing_amount)
				templist.append(ing_unit)
				storedforread.append(templist)
			...
			if checkforpicture==1:
				initial = {'recipe_title': title ,'recipe_instructions' : instructions }
				form = AddRecipeForm(initial=initial)
				# print('LOOOOOOK',len(storedforread))
				formset = AddRecipeFormIngSet(initial=[{'ingredient':storedforread[x][0], 'ingredient_amount': storedforread[x][1], 'ingredient_unit' : storedforread[x][2]} for x in range(len(storedforread))])
